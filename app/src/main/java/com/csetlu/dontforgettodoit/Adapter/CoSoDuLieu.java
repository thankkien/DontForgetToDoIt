...
public class CoSoDuLieu extends SQLiteOpenHelper {
    ...
    private static final int PHIENBAN = 4;
    ...
    private static final String TAOBANG = "CREATE TABLE " + BANG + " (" + MACV + " INTEGER PRIMARY KEY AUTOINCREMENT, " + CONGVIEC + " TEXT, " + NGAY + " TEXT, " + THOIGIAN + " TEXT)";
    ...
    public int suaCongViec(ViecCanLamM congViec) {
        ContentValues banGhi = new ContentValues();
        banGhi.put(CONGVIEC, congViec.layCV());
        banGhi.put(NGAY, congViec.layNgay());
        banGhi.put(THOIGIAN, congViec.layThoiGian());
        return db.update(BANG, banGhi, MACV + "=?", new String[]{String.valueOf(congViec.layMaCV())});
    }

    public int xoaBanGhi(int maCV) {
        return db.delete(BANG, MACV + "=?", new String[]{String.valueOf(maCV)});
    }
}
